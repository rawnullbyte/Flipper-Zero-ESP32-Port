...
static const char* parse_nrf_connect(FindMy* app, const char* path) {
    ...
    do {
        ...
        if(error) break;
        for(int i = 0; i < 3; i++) { uint8_t tmp = mac[i]; mac[i] = mac[5-i]; mac[5-i] = tmp; }
...
static const char* parse_open_haystack(FindMy* app, const char* path) {
    ...
    do {
        ...
        for(int i = 0; i < 3; i++) { uint8_t tmp = app->state.mac[i]; app->state.mac[i] = app->state.mac[5-i]; app->state.mac[5-i] = tmp; }
